...
public class FavoritesAlertManager implements DiagnosticManager {
    ...
    public FavoritesAlertManager(DiagnosticActivity context,
    ...
    public void showAlert() {
        ...
        builder.setTitle(mContext.getResources().getString(
                R.string.favorites_alert_label));
    ...
    private void createAndAddRowToTable(final LinearLayout favoritesLayout,
        ...
        final LinearLayout favoritesTable = (LinearLayout) favoritesLayout
                .findViewById(R.id.favoritesAlertTable);
        ...
        if (MessageAnalyzer.isDiagnosticRequest(reqMessage)) {
            ...
            final TextView nameData = (TextView) row
                    .findViewById(R.id.favoritesNameData);
            TextView nameHeader = (TextView) favoritesLayout
                    .findViewById(R.id.nameHeader);
            headerMatcher.put(nameHeader, nameData);
            nameData.setText(Formatter.getNameOutput(req));

            final TextView busData = (TextView) row
                    .findViewById(R.id.favoritesBusData);
            TextView busHeader = (TextView) favoritesLayout
                    .findViewById(R.id.busHeader);
            headerMatcher.put(busHeader, busData);
            busData.setText(Formatter.getBusOutput(req));

            final TextView idData = (TextView) row
                    .findViewById(R.id.favoritesIdData);
            TextView idHeader = (TextView) favoritesLayout
                    .findViewById(R.id.idHeader);
            headerMatcher.put(idHeader, idData);
            idData.setText(Formatter.getIdOutput(req));

            final TextView modeData = (TextView) row
                    .findViewById(R.id.favoritesModeData);
            TextView modeHeader = (TextView) favoritesLayout
                    .findViewById(R.id.modeHeader);
            headerMatcher.put(modeHeader, modeData);
            modeData.setText(Formatter.getModeOutput(req));

            final TextView pidData = (TextView) row
                    .findViewById(R.id.favoritesPidData);
            TextView pidHeader = (TextView) favoritesLayout
                    .findViewById(R.id.pidHeader);
            headerMatcher.put(pidHeader, pidData);
            pidData.setText(Formatter.getPidOutput(req));

            final TextView payloadData = (TextView) row
                    .findViewById(R.id.favoritesPayloadData);
            TextView payloadHeader = (TextView) favoritesLayout
                    .findViewById(R.id.payloadHeader);
            headerMatcher.put(payloadHeader, payloadData);
            payloadData.setText(Formatter.getPayloadOutput(req));

            final TextView frequencyData = (TextView) row
                    .findViewById(R.id.favoritesFrequencyData);
            TextView frequencyHeader = (TextView) favoritesLayout
                    .findViewById(R.id.frequencyHeader);
            headerMatcher.put(frequencyHeader, frequencyData);
            frequencyData.setText(Formatter.getFrequencyOutput(req));

        } else {

            final TextView commandData = (TextView) row
                    .findViewById(R.id.favoritesCommandData);
            TextView commandHeader = (TextView) favoritesLayout
                    .findViewById(R.id.commandHeader);
            headerMatcher.put(commandHeader, commandData);
            commandData.setText(Formatter
                    .getCommandOutput((Command) reqMessage));
        }

        // the columns of the table don't line up if you let android do it
        // "on layout" so this lets android create the layout, then adjust
        // according to the properties in the xml, then the rest of the rows'
        // columns will adjust to match
        Iterator<Map.Entry<TextView, TextView>> it = headerMatcher.entrySet()
                .iterator();
        while (it.hasNext()) {
            final Map.Entry<TextView, TextView> pair = it.next();
            pair.getKey().addOnLayoutChangeListener(
                    new View.OnLayoutChangeListener() {
                        @Override
                        public void onLayoutChange(View v, int left, int top,
                                int right, int bottom, int oldLeft, int oldTop,
                                int oldRight, int oldBottom) {
                            LayoutParams param = pair.getKey()
                                    .getLayoutParams();
                    ...
                    public void onClick(View v) {
                        ...
                        if (mDisplayCommands) {
                            ...
                            message = mContext
                                    .getResources()
                                    .getString(
                                            R.string.delete_favorite_command_verification);
                        } else {
                            title = "Delete Request?";
                            message = mContext
                                    .getResources()
                                    .getString(
                                            R.string.delete_favorite_request_verification);
                        }
